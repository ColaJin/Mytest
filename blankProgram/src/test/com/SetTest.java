 ...
 *                      LinkedHashSet：作为HashSet子类；遍历其内部数据时，可以按照调价的顺序遍历
 ...
 *  重写两个方法的小技巧：对象用作equals()方法比较的Field，都应该用来计算hashCode值
 */
public class SetTest {
     ...
     *
     * 对于添加成功的情况2和情况3而言：元素a 已经存在指定索引位置上的数据以链表方式存储
     *
    ...
    //LinkedHashSet的使用
    //LinkedHashSet作为HashSet的子类，添加数据的同时，每个数据还维护了两个引用，记录次数据前一个数据和后一个数据
